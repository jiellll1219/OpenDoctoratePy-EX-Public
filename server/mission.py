...
class mission_manger:
    def __init__(self):
        ...
        self.mapping = str.maketrans("abcdefghijklmnopqrstuvwxyz", "ABCDEFGHIJKLMNOPQRSTUVWXYZ")
        self.result = {
            "playerDataDelta": {
                "modified": {},
                "deleted": {}
    ...
    def ConfirmMission(self):
        ...
        # 任务类型字符串转大写
        mission_type = str((mission_id.split("_")[0]).translate(mapping))
        ...
        for key, mission_id_data in mission_data.items():
            ...
            if key == mission_id:
                ...
                if state:
                    ...
                    # 设置下一个任务状态为可见
                    self.mission_state_check(mission_type, key)
        #TODO:检查任务类型并把状态设置为可见
    ...
    def AutoConfirmMissions(self):
        ...
        for mission_id, mission_data_item in mission_data.items():
            ...
            result["playerDataDelta"]["modified"]["mission"]["missions"] = {}
            result["playerDataDelta"]["modified"]["mission"]["missions"][mission_type] = {}
            result["playerDataDelta"]["modified"]["mission"]["missions"][mission_type].update(mission_data)
    ...
    def mission_state_check(self, mission_type: str, mission_id: str):
        '''
        用于每日任务与每周任务的状态更新

        :param mission_type: 要检查的任务状态类型，只能是"DAILY"或"WEEKLY"
        :param mission_id: 可选，任务id，用于单个任务确认函数
        '''
        ...
        mapping = self.mapping
        daily_start_list = self.daily_start_list
        weekly_start_list = self.weekly_start_list
        
        # 根据任务类型选择排除列表
        match mission_type:
            case "DAILY":
                start_list = set(daily_start_list)
            case "WEEKLY":
                start_list = set(weekly_start_list)
            case _:
                return 0
        ...
        if mission_id is not None:
            ...
            if new_id in start_list:
                ...
                for value in mission_data[new_id]["progress"]:
                    state = True
            ...
            cnt = len(mission_data.keys()) + 1
            mission_str = mission_type.translate(mapping) + "_"

            #TODO: 需要获取每日任务的大类，即任务id前两位，从48到63
            # todo = None
            # match mission_str:
            #     case "DAILY":   
            #         mission_int = todo.split("_")[:2]
            #     case "WEEKLY":
            #         mission_int = 700
            ...
            for i in range(1, cnt):
                new_id = mission_str + str(mission_int + i)
                if new_id in start_list:
                    break
                else:
                    state = True
                    progress_data = mission_data[new_id]["progress"]
                    # 获取 progress 中全部元素，并减 1 作为偏移量使用
                    offset = len(progress_data) - 1
                    # 循环每一个任务的要求并进行比较
                    for i in range(offset):
                        data = progress_data[i]
                        # 如果当前值小于目标值，则把任务完成状态设为 False
                        if data["value"] >= data["target"]:
                            continue
                        else:
    ...
    def re_set_state(mission_type: str):
        '''
        用于重置任务状态
        ...
        # default_mission = read_json()
        # re_set_mission = sync_data["user"]["mission"]["missions"][mission_type]
        # default_mission_data = default_mission[mission_type]

        # re_set_mission = default_mission_data
